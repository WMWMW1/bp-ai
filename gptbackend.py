...
# Function to either get user input or use demo data for company info
def get_company_info(demo_mode=False):
    """Fetches company information either through user input or demo data based on the demo mode flag."""
    if demo_mode:
    ...
    print_company_info(user_info)  # Print company information in a formatted manner
    return user_info

def print_company_info(company_info):
    """Prints the provided company information in a structured format."""
    print("\nCompany Information:\n")
...
def print_summary(title, summary):
    """Prints a formatted summary for a specific section of the business plan."""
    print(f"### Summary for {title} ###\n{summary}\n\n" + "-"*50 + "\n")

def main():
    """Main function to drive the business plan generation script."""
    ...
    # Start building the Markdown content for the business plan
    markdown_content = f"# Business Plan for {company_info['name']}\n"

    # Generate content for each part of the business plan
